...
当前产品版本:`0.3.0-alpha.20260616`。它表示互动系统已经进入 App,但阅读体验、书架/书城逻辑、我的页和自动化验收仍在打磨中,还不是 1.0 成熟版。
...
这次拆分是阅读器产品化阶段的基础。后面加划线、笔记、TTS、词典、竖排阅读时,优先改对应页面或组件,不要再把新功能塞回 `AppRoot.kt`。

## 版本规则

**结论:产品版本给用户看,工程阶段给开发者排工单。**

| 名称 | 用途 | 示例 |
| --- | --- | --- |
| 产品版本 | 写入 APK、README、Release,表示成熟度 | `0.3.0-alpha.20260616` |
| 工程阶段 | 平台书和 TODO 里排任务,表示内部迭代方向 | `v2.2 笔记互动` |

当前版本含义:

- `0.3.0`:已完成登录、书架、阅读、AI、划线、笔记、段评、点赞的主要链路;
- `alpha`:功能能跑,但 UI 手感和自动化验收还没达到可长期使用标准;
- `20260616`:里程碑日期,方便从构建号知道是哪一批。

1.0 的定义:普通读者不用说明书也能完成登录、找书、加入书架、阅读、划线、笔记、段评、AI 问答、查看个人记录,并且主要链路有自动化测试兜底。
...
- `SectionHeader` / `SearchField` / `BrTextField` / `BrButton` / `BannerCard` / `InfoCard` / `QuickEntryGrid` / `EntryCard` / `MiniPlayerBar`:页面区块、搜索、输入、按钮、内容卡片、入口卡片、播放条统一组件。
- `BrActionDock` / `BrDockAction` / `AiPromptChip` / `AiInputBar`:阅读选择工具条和 AI 临时窗口统一组件。
